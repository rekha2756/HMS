...
// Only super-admin can create new tenants (you may add a role check)
router.post('/create', authMiddleware, async (req, res) => {
  ...
  // Validate subdomain format (only alphanumeric and underscore)
  ...
  try {
    // 1. Create the new schema (database)
    await query('', `CREATE DATABASE ${schemaName}`); // Use a special connection without db name
    console.log(`Created database: ${schemaName}`);

    // 2. Copy tables from a template schema (e.g., 'hms_template')
    // We assume you have a schema 'hms_template' with all tables (no data)
    ...
    // 3. Insert the hospital admin user
    ...
    // 4. Record the tenant in central registry
